function exp_array=expand_array(MAP,node_x,node_y,hn,xTarget,yTarget,CLOSED,MAX_X,MAX_Y)
    %把某个点周围的所有不在关闭列表中的点，都计算GHF，然后返回它们向量的集合
        ...
        for j= 1:-1:-1
            if (k~=j || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    flag=1;
					if(abs(k) == 1 && abs(j) == 1)
						if(MAP(node_x, s_y) == -1 || MAP(s_x, node_y) == -1)
							flag = 0;
						end
					end
                    for c1=1:c2
                    ...
                    if (flag == 1)
                        ...
                        potential=Astar_potential(MAP,s_x,s_y);
                        exp_array(exp_count,4) = distance(xTarget,yTarget,s_x,s_y)+...
                           potential;%distance between node and goal
